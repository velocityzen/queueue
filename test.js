...
test.cb("binds the context and method", (t) => {
  ...
  const q = new Q().bind(obj, "get").on("drain", () => t.end());
...
test.cb("checks the concurency", (t) => {
  ...
  q.bind(get).push([{ args: [1] }, { args: [2] }, { args: [3] }]);
});

test.cb("aborts the queueue after first result", (t) => {
  t.plan(1);

  const q = new Q(2).on("error", () => q.abort()).on("drain", () => t.end());
  ...
  q.bind(obj, "get").push([{ args: [1] }, { args: [2] }, { args: [3] }]);
});

test.cb("catches the error", (t) => {
  t.plan(4);
  ...
  q.bind(null, task).push([{ args: [1] }, { args: [0] }, { args: [2] }]);
});
